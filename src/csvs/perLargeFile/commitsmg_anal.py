...
def main():
    ...
    folder_path_large_files = os.path.join("src", "csvs", "perLargeFile", "tiny")
...
Porcentis
Commit Operation -> {percent(ttiny_commit,total):.2f}%
Build Configuration -> {percent(ttiny_build,total):.2f}%
...
Optimizing -> {percent(ttiny_opt,total):.2f}%
Test -> {percent(ttiny_test,total):.2f}%
