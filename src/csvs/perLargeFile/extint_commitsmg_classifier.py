...
classs = ['Test','Library']
for clas in classs:
    extint = pd.read_csv(f"src/csvs/perLargeFile/extint/{clas}_Sort.csv")
    extint = extint.sort_values(by='Project', ascending=True)
    extint_list = []
    ...
    csv_name = ''
    for i in range(len(extint)):
        if i==0 or extint['Project'].loc[i] != extint['Project'].loc[i-1]:
            csv_name = extint['Project'].loc[i]
            try:
        ...
        if extint['Internal or External'].loc[i] != "Internal":
            continue

        commits_unclassified_total = []


        large_file = extint['File Name'].loc[i]
        filtered_df = df[df['File Name'] == large_file]


        output_csv_file = f'src/csvs/perLargeFile/extint/{csv_name}/{large_file}/{csv_name}{large_file.strip().replace(" ", "")}.csv'
